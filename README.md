...
界面顶栏右侧显示当前版本号, 有新版本时替换为可点击的更新入口; 更新窗口支持检查, 下载, 跳过版本, 查看 release notes, 并在下载完成后由用户决定何时 "重启并更新". 自动更新只在安装了发布产物时可用, 本地开发构建显示 `dev-build` 且不参与更新检查.
...
本项目使用 AGPL-3.0-or-later 协议发布, 详见 [LICENSE](LICENSE).

如果你基于本工具修改, 分发, 或作为网络服务提供功能, 需要按协议公开对应源代码.

样例工程和评测脚本默认随仓库按同一协议发布.
