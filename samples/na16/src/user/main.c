...
idata u8 led7_tick = 0;
// 504 触发后保留一段 L8 提示态, 题面只要求指示灯 3s 后熄灭.
bit fk8 = 0;
idata u8 fk8_tick = 0;
#define FK8_TIME 200
...
void sg_proc() {
	...
	if (s_param == S_TYPE_CIRCLE) {
		...
		tf = 100.0 * vol_liq / 10.0 / (4.0 / 3 * 3.14 * r_param_10x / 10.0 * r_param_10x / 10.0 * r_param_10x / 10.0);
		...
		// 圆柱体体积按 pi * r * r * h 计算, 显示值保留到 0.1L.
		vol_liq = 10.0 * 3.14 * r_param_10x / 10.0 * r_param_10x / 10.0 * liq_height_100x / 100.0;
		...
		vol_liq = 10.0 * w_param_10x / 10.0 * l_param_10x / 10.0 * liq_height_100x / 100.0;
		tf = 100.0 * liq_height_100x / 10 / h_param_10x;
	}
	if (vol_liq < 0) vol_liq = 0;
	// 数码管显示到 0.1, 这里统一做四舍五入.
	vol_liq_10x = vol_liq + 0.5;
	if (tf > 100) tf = 100;
	if (tf < 0) tf = 0;
	used_ratio = tf + 0.5;
	if (used_ratio > 100) used_ratio = 100;
	vol_spare_ratio = 100 - used_ratio;
...
void timer1_isr() interrupt 3 {
	...
	if (++freq_tick == FREQ_TIME) {
		...
		// 504 直接比较相邻两次 1s 频率采样的差值
		if (freq_queue_idx_initial >= 2) {
			prev_freq = freq_queue[(freq_queue_idx + FREQ_QUEUE_SIZE - 1) % FREQ_QUEUE_SIZE];
			freq_change_1s = (long)freq - (long)prev_freq;
			if (freq_change_1s > 1000 || freq_change_1s < -1000) {
				error_isr_504 = 1;
				error_isr_val_504 = freq_change_1s;
				fk8_elapsed = 0;
				fk8 = 1;
			} else {
				error_isr_504 = 0;
