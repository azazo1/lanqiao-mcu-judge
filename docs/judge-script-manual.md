...
例如 `na16` 的 `503` 趋势异常场景里, 频率并不是每 `1000ms` 恰好切一次, 而是每 `1200ms` 才切一次. 这样做的目的是:

- 避免把 1500 -> 1300 -> 1000 -> 800 -> 500 这些翻转点卡在 1s 采样边界上.
- 给每个目标频率至少留出 `200ms` 的稳定观察余量, 防止脚本把"采样相位边界问题"误判成实现错误.
- 这样推导出来的可接受时间窗, 才是由题面采样规则和场景相位共同决定的, 而不是由某个 `hex` 的偶然输出决定的.
