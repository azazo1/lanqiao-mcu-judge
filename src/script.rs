...
fn register_api(engine: &mut Engine, sim: &Arc<Mutex<Simulator>>) {
    ...
    engine.register_fn("set_key", move |name: ImmutableString, pressed: bool| -> Result<(), Box<EvalAltResult>> {
        sim_key
            .lock()
            .map_err(|_| runtime_error("仿真器锁已损坏"))?
    ...
    engine.register_fn("tap_key", move |name: ImmutableString, hold_ms: i64| -> Result<(), Box<EvalAltResult>> {
        let hold_ms = u64::try_from(hold_ms).map_err(|_| runtime_error("hold_ms 参数必须 >= 0"))?;
    ...
    engine.register_fn("set_rtc", move |hour: i64, minute: i64, second: i64| -> Result<(), Box<EvalAltResult>> {
    ...
    engine.register_fn("set_temperature_c", move |temp: i64| -> Result<(), Box<EvalAltResult>> {
        sim_temp
            .lock()
            .map_err(|_| runtime_error("仿真器锁已损坏"))?
    ...
    engine.register_fn("set_distance_cm", move |distance: i64| -> Result<(), Box<EvalAltResult>> {
        sim_distance
            .lock()
            .map_err(|_| runtime_error("仿真器锁已损坏"))?
    ...
    engine.register_fn("set_frequency_hz", move |hz: i64| -> Result<(), Box<EvalAltResult>> {
        sim_frequency
            .lock()
            .map_err(|_| runtime_error("仿真器锁已损坏"))?
    ...
    engine.register_fn("set_voltage", move |name: ImmutableString, voltage: rhai::FLOAT| -> Result<(), Box<EvalAltResult>> {
        sim_voltage
            .lock()
            .map_err(|_| runtime_error("仿真器锁已损坏"))?
    ...
    engine.register_fn("uart_write", move |text: ImmutableString| -> Result<(), Box<EvalAltResult>> {
        sim_uart
            .lock()
            .map_err(|_| runtime_error("仿真器锁已损坏"))?
    ...
    engine.register_fn("uart_take", move || -> Result<ImmutableString, Box<EvalAltResult>> {
    ...
    engine.register_fn("display_text", move || -> Result<ImmutableString, Box<EvalAltResult>> {
    ...
    engine.register_fn("snapshot_text", move || -> Result<ImmutableString, Box<EvalAltResult>> {
    ...
    engine.register_fn("led_on", move |index: i64| -> Result<bool, Box<EvalAltResult>> {
    ...
    engine.register_fn("assert", move |cond: bool, message: ImmutableString| -> Result<(), Box<EvalAltResult>> {
        if cond {
            return Ok(());
        }
    ...
    engine.register_fn("assert_eq_str", move |actual: ImmutableString, expected: ImmutableString, label: ImmutableString| -> Result<(), Box<EvalAltResult>> {
        if actual == expected {
            return Ok(());
        }
        Err(runtime_error(format!("{label}: 期望 `{expected}`, 实际 `{actual}`")))
    });

    engine.register_fn("assert_eq_int", move |actual: i64, expected: i64, label: ImmutableString| -> Result<(), Box<EvalAltResult>> {
        if actual == expected {
            return Ok(());
        }
        Err(runtime_error(format!("{label}: 期望 {expected}, 实际 {actual}")))
