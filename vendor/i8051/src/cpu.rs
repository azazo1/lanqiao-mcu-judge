...
impl fmt::Display for Direct {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::Sfr(SFR_AUXR1) => write!(f, "AUXR1"),
...
impl Cpu {
    ...
    pub fn register_set(&mut self, register: Register, value: u16) {
        match register {
            ...
            Register::DPL => self.set_dpl(value as u8),
            Register::DPH => self.set_dph(value as u8),
    ...
    fn dptr_bank_1_active(&self) -> bool {
        self.auxr1 & 0x01 != 0
    }

    pub fn dpl(&self) -> u8 {
        if self.dptr_bank_1_active() {
            self.dpl1
        } else {
            self.dpl0
        }
    ...
    pub fn sfr_set(&mut self, addr: u8, value: u8, ctx: &mut impl CpuContext) {
        match addr {
            SFR_A => self.a = value,
            SFR_B => self.b = value,
            SFR_DPH => self.set_dph(value),
            SFR_DPL => self.set_dpl(value),
            SFR_AUXR1 => self.auxr1 = value,
...
op! {
    ...
    OP SUBB "A,#{imm8:02X}" 0b10010100 imm8 => {(A, C, OV, AC) = sub_with_borrow(A, imm8, C); PC+=2};
    OP SUBB "A,{direct}" 0b10010101 direct => {(A, C, OV, AC) = sub_with_borrow(A, DATA[direct], C); PC+=2};
    OP SUBB "A,@R{x}" 0b10010110 -x 0b1 => {(A, C, OV, AC) = sub_with_borrow(A, IDATA[R[x]], C); PC+=1};
    OP SUBB "A,R{x}" 0b10011000 -x 0b111 => {(A, C, OV, AC) = sub_with_borrow(A, R[x], C); PC+=1};
...
fn add_with_carry(a: Reg8, b: Reg8, c: bool) -> (Reg8, bool, bool, bool) {
    ...
    let ov = ((a.0 ^ result) & (b.0 ^ result) & 0x80) != 0;

    (Reg8(result), (sum >> 8) != 0, ov, ac)
}

#[inline(always)]
fn sub_with_borrow(a: Reg8, b: Reg8, c: bool) -> (Reg8, bool, bool, bool) {
    let borrow_in = c as u8;
    let result = a.0.wrapping_sub(b.0).wrapping_sub(borrow_in);
    let borrow = (a.0 as u16) < (b.0 as u16 + borrow_in as u16);
    let ov = ((a.0 ^ b.0) & (a.0 ^ result) & 0x80) != 0;
    let ac = (a.0 & 0x0F) < ((b.0 & 0x0F) + borrow_in);
    (Reg8(result), borrow, ov, ac)
}
...
#[test]
fn auxr1_switches_active_dptr_bank() {
    let mut cpu = Cpu::new();
    let mut ctx = ();

    cpu.sfr_set(SFR_DPL, 0x34, &mut ctx);
    cpu.sfr_set(SFR_DPH, 0x12, &mut ctx);
    assert_eq!(cpu.dptr(), 0x1234);

    cpu.sfr_set(SFR_AUXR1, 0x01, &mut ctx);
    assert_eq!(cpu.dptr(), 0x0000);

    cpu.sfr_set(SFR_DPL, 0x78, &mut ctx);
    cpu.sfr_set(SFR_DPH, 0x56, &mut ctx);
    assert_eq!(cpu.dptr(), 0x5678);

    cpu.sfr_set(SFR_AUXR1, 0x00, &mut ctx);
    assert_eq!(cpu.dptr(), 0x1234);

    cpu.sfr_set(SFR_AUXR1, 0x01, &mut ctx);
    cpu.register_set(Register::DPTR, 0x9ABC);
    assert_eq!(cpu.dptr(), 0x9ABC);
